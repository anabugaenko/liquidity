...
def _plot_impact_imb_curves(suptitle: str,
...
def _plot_response_vs_imbalance(fig, df_, T, xlim=None, ylim=None,
                                imbalance_col='vol_imbalance', num_q=31, marker='o', alpha=0.5, color=None):
    ax = fig.gca()
    ...
    plt.plot(df_[imbalance_col], df_[f"R"], label=f'T={T}', marker=marker, linewidth=0,
             markerfacecolor='white', markersize=5, markeredgecolor=color, alpha=0.5)
    plt.legend(loc='best')
...
def plot_fitted_model_with_data(data_binned: pd.DataFrame, durations,
                                popt, xlim, ylim,
                                color_fitted_line, color_markers_line,
                                color_markers, title=None, alpha=0.7,
                                imbalance_col='vol_imbalance', y_reflect=False,
                                qmax=0.005, save=False, filename=None):
    if popt is None:
        print('Not solved')
        return
    ...
    if title is not None:
        fig.suptitle(title)
    ax = sns.lineplot(x='x_scaled', y='y_scaled', data=data_binned, style="T", color=color_markers_line,
                      markers=MARKERS_LIST, legend='full', markerfacecolor='none', markevery=0.000005,
    ...
    ylabel = r"$\it{R(\Delta{V},T)/\mathcal{R}(1)T^{\chi_v}}$" if imbalance_col == 'vol_imbalance' else \
        r"$\it{R(\mathcal{E},T)/\mathcal{R}(1)T^{\chi}}$"
...
def plot_renorm_fits(data_norm, durations, fit_param, ylim=None, xlim=None,
                     imbalance_col='vol_imbalance', save=False, filename=None):
    pylab.rcParams['xtick.major.pad'] = '8'
    pylab.rcParams['ytick.major.pad'] = '8'
    ...
    for indx, T in enumerate(durations):
        ...
        if T in fit_param.keys():
            ...
            _plot_response_vs_imbalance(fig, data_norm[data_norm['T'] == T], T=T,
                                        imbalance_col=imbalance_col, num_q=50,
                                        marker=MARKERS_LIST[indx], alpha=1, color=COLORS_LIST[indx])

            xlabel = r"$\it{\Delta{V}'}/V_D$" if imbalance_col == 'vol_imbalance' \
                else r"$\it{\Delta{\mathcal{E}}'}/\mathcal{E}_D$"
            ylabel = r"$\it{R(\Delta{V},T)}/\mathcal{R}(1)$" if imbalance_col == 'vol_imbalance' \
            ...
            if popt is not None:
                plt.plot(q, fit_param[T][2](np.array([q, T]), *popt),
                         '--',
                         label='', color=COLORS_LIST[indx], alpha=0.5)
                if ylim is not None:
                    plt.ylim(-ylim, ylim)
                if xlim is not None:
                    plt.xlim(-xlim, xlim)
...
def plot_collapsed_fitted_func(fit_param,
                               line_color,
                               markers_color,
                               ylim=None, xlim=None,
                               imbalance_col='vol_imbalance',
                               renorm=True, durations=None,
                               master_curve=None,
        ...
        if T in fit_param.keys():
            ...
            if popt is not None:
                plt.plot(q, fit_param[T][2](np.array([q, T]), *popt),
                         ...
                         markevery=0.05, ms=3.5,
                         label='x', color=line_color, alpha=ALPHA_LIST[indx],
    ...
    if renorm:
        xlabel = X_AXIS_LABELS['vol_renorm'] if imbalance_col == 'vol_imbalance' else X_AXIS_LABELS['sign_renorm']
        ylabel = r"$\it{R(\Delta V^\prime,T)/\mathcal{R}(1)T^{\chi_v}}$" if imbalance_col == 'vol_imbalance' else \
            r"$\it{R(\Delta \mathcal{E}^\prime,T)/\mathcal{R}(1)T^\chi}$"
    else:
        xlabel = X_AXIS_LABELS['vol'] if imbalance_col == 'vol_imbalance' else X_AXIS_LABELS['sign']
        ylabel = r"$\it{R(\Delta V^\prime,T)}$" if imbalance_col == 'vol_imbalance' else \
            r"$\it{R(\Delta \mathcal{E}^\prime,T)}$"
