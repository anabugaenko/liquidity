...
def fit_and_save(stock_to_series_dict, filename):
    """
...
def get_fitting_params(fit_objects: Dict[str, Fit], distribution: str) -> pd.DataFrame:
    ...
    for stock_name, fit in fit_objects.items():
        ...
        base_result.update({
            'xmin': fit.xmin,
            'KS Distance': getattr(fit, distribution).D if hasattr(fit, distribution) else np.nan
        })
...
def distribution_compare(fit_objects: Dict[str, Fit], distribution: str) -> pd.DataFrame:
    ...
    for stock_name, fit in fit_objects.items():
        ...
        base_result.update({
            ...
            'KS Distance (' + distribution + ')': getattr(fit, distribution).D if hasattr(fit,
                                                                                          distribution) else np.nan,
